import { Component, signal, computed, inject, AfterViewInit, OnDestroy, ChangeDetectionStrategy } from '@angular/core';
...
@Component({
  ...
  imports: [FormsModule, RouterLink, TranslatePipe, ModalComponent, ZoomControlsComponent, WeekNavigationComponent, TaskCardComponent],
...
export class TimelineComponent implements AfterViewInit, OnDestroy {
  ...
  // Estados de Modales delegados al UiService
  get isTaskModalOpen() { return this.uiService.isTaskModalOpen; }
  get isUserModalOpen() { return this.uiService.isUserModalOpen; }
  get isProjectModalOpen() { return this.uiService.isProjectModalOpen; }
  ...
  readonly dependencyLines = computed(() => {
    ...
    const lines: { path: string; id: string; userId: string; colorClass: string }[] = [];
      ...
      if (taskB.dependencies) {
        for (const predId of taskB.dependencies) {
          ...
          if (taskA) {
            ...
            const y1 = offsetA + taskA.top + 28; // 28px es la mitad de la altura de la tarea (56px)

            const x2 = (taskB.left / 100) * gridWidth;
            const y2 = offsetB + taskB.top + 28;
  ...
  readonly activeProjectsCount = computed(() => this.timelineService.projects().filter(p => p.isActive !== 0).length);
  readonly activeUsersCount = computed(() => this.activeUsers().length);
  readonly totalPlannedHours = computed(() => this.timelineService.tasks().reduce((sum, t) => sum + Number(t.duration), 0));

  // Estadísticas rápidas
  readonly totalTasks = computed(() => this.timelineService.tasks().length);
  readonly totalUsers = computed(() => this.activeUsers().length);
  readonly busyUsersCount = computed(() => {
    const nonAdminUserIds = new Set(this.activeUsers().map(u => u.id));
    const activeUserIds = new Set(this.timelineService.tasks().filter(t => nonAdminUserIds.has(t.userId)).map(t => t.userId));
  ...
  deleteTask(taskId: string, event: Event) {
    ...
    const confirmMsg = isEn ? 'Are you sure you want to delete this task?' : '¿Estás seguro de que quieres eliminar esta tarea?';
  ...
  deleteUser(userId: string, name: string) {
    const isEn = this.translationService.currentLang() === 'en';
    const msg = isEn 
      ? `Are you sure you want to delete ${name}? Their tasks will also be deleted.`
      : `¿Estás seguro de eliminar a ${name}? También se borrarán sus tareas.`;
    this.uiService.confirm(
  ...
  deleteProject(projectId: string, name: string, event: Event) {
    ...
    const msg = isEn 
      ...
      : `¿Estás seguro de eliminar el proyecto "${name}"? También se borrarán todas sus tareas asociadas.`;
    this.uiService.confirm(
  ...
  getDayDate(dayIndex: number): Date {
    return addDays(this.currentWeekMonday(), dayIndex);
  }
  ...
  getDayStatusClass(dayIndex: number): string {
    ...
    const d1 = new Date(dayDate.getFullYear(), dayDate.getMonth(), dayDate.getDate()).getTime();
